...
async function fetchExample(search) {
    try {
   	...
   	/* response.ok ist true, wenn die Antwort einen "guten" Statuscode hat, z.B. 200. Beim Statuscode 404 ("nicht gefunden") wäre ok false, aber das würde keine Error erzeugen, da die Anfrage ja korrekt funktioniert hat, auch wenn mir die Antwort nicht gefällt. D.h. hier müssen wir selbst auf einen guten Statuscode (der die gewünschten Daten liefert) prüfen und dann selbst einen Fehler werfen. Die Zeilen nach if werden dann nicht mehr ausgeführt, und JavaScript spingt in den catch-Block. */
   	 if (!response.ok) {
   	...
   	/* Hier solle nicht nur in der Konsole etwas ausgegeben werden, sondern es sollte z.B. den Usern angezeigt werden, dass etwas mit der Anfrage schief ging. */
   	 console.log(error);
    }
